...
static std::tuple<Tensor, Tensor, Tensor, Tensor> _embedding_bag_mps_impl(
  ...
  for (const auto dim : c10::irange(weight.dim())) {
    params.weight_strides[dim] = c10::safe_conv<uint32_t, int64_t>(weight.stride(dim));
    params.output_strides[dim] = c10::safe_conv<uint32_t, int64_t>(output.stride(dim));

    if (mode == EmbeddingBagMode::MAX) {
      params.max_indices_strides[dim] = c10::safe_conv<uint32_t, int64_t>(max_indices.stride(dim));
    }
  }
